...
fn test_checksum_sha256() {
    ...
    engraver()
        ...
        // SHA-256 of "Hello, World!\n"
        .stdout(predicate::str::contains("c98c24b677eff44860afea6f493bbaec5bb1c4cbb209c6fc2bbb47f66ff2ad31"));
}
...
fn test_checksum_md5() {
    ...
    engraver()
        .args(["checksum", test_file.to_str().unwrap(), "--algorithm", "md5"])
        .assert()
        .success()
...
fn test_checksum_sha512() {
    ...
    engraver()
        .args(["checksum", test_file.to_str().unwrap(), "--algorithm", "sha512"])
        .assert()
        .success()
...
fn test_checksum_crc32() {
    ...
    engraver()
        .args(["checksum", test_file.to_str().unwrap(), "--algorithm", "crc32"])
        .assert()
        .success()
...
fn test_checksum_missing_file() {
    engraver()
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Failed to validate source")
...
fn test_checksum_invalid_algorithm() {
    ...
    engraver()
        .args(["checksum", test_file.to_str().unwrap(), "--algorithm", "invalid"])
        .assert()
        .failure()
...
fn test_write_invalid_target() {
    ...
    engraver()
        .args(["write", test_file.to_str().unwrap(), "/dev/nonexistent_device"])
        .assert()
        .failure();
}
...
fn test_write_yes_flag() {
    ...
    engraver()
        .args(["write", test_file.to_str().unwrap(), "/dev/nonexistent", "--yes"])
        .assert()
        .failure();
}
...
fn test_write_verify_flag() {
    ...
    engraver()
        .args(["write", test_file.to_str().unwrap(), "/dev/nonexistent", "--verify"])
        .assert()
        .failure();
}
...
fn test_write_block_size_flag() {
    ...
    engraver()
        .args(["write", test_file.to_str().unwrap(), "/dev/nonexistent", "--block-size", "1048576"])
        .assert()
        .failure();
}
...
fn test_checksum_correct_for_binary_data() {
    ...
    engraver()
        ...
        .stdout(predicate::str::contains("40aff2e9d2d8922e47afd4648e6967497158785fbd1da870e7110266bf944880"));
}
...
fn test_invalid_args_exit_code() {
    engraver()
        .arg("write")
        .assert()
        .failure(); // Just check it fails, exit code varies
}
