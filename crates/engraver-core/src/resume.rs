...
impl WriteCheckpoint {
    ...
    pub fn new(
        ...
        let total_blocks = source_info.size.map(|size| {
            size.div_ceil(config.block_size as u64)
        });
...
impl CheckpointManager {
    ...
    pub fn find_checkpoint(&self, source_path: &str, target_path: &str) -> Result<Option<WriteCheckpoint>> {
...
mod tests {
    ...
    fn test_checkpoint_creation() {
        ...
        let checkpoint = WriteCheckpoint::new(&source_info, "/dev/sdb", 32 * 1024 * 1024 * 1024, &config);
    ...
    fn test_checkpoint_progress_update() {
        let source_info = create_test_source_info();
        let config = create_test_config();
        let mut checkpoint = WriteCheckpoint::new(&source_info, "/dev/sdb", 32 * 1024 * 1024 * 1024, &config);
