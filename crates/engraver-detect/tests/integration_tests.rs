...
fn test_drive_safety_combinations() {
    ...
    assert!(drive.is_safe_target(), "Removable non-system drive should be safe");

    // removable=true, is_system=true -> UNSAFE
    let drive = Drive::new("/dev/sdb")
        .with_removable(true)
        .with_system(true, Some("Test".to_string()));
    assert!(!drive.is_safe_target(), "Removable system drive should not be safe");

    // removable=false, is_system=false -> UNSAFE
    let drive = Drive::new("/dev/sda")
        .with_removable(false)
        .with_system(false, None);
    assert!(!drive.is_safe_target(), "Non-removable drive should not be safe");

    // removable=false, is_system=true -> UNSAFE
    let drive = Drive::new("/dev/sda")
        .with_removable(false)
        .with_system(true, Some("Root".to_string()));
    assert!(!drive.is_safe_target(), "Non-removable system drive should not be safe");
}
...
fn test_format_bytes_edge_cases() {
    ...
    assert_eq!(format_bytes(u64::MAX), format!("{:.1} TB", u64::MAX as f64 / (1024.0 * 1024.0 * 1024.0 * 1024.0)));
}
...
fn test_system_mount_point_detection() {
    ...
    for mount in system_mounts {
        assert!(
            ...
            "{} should be detected as system mount",
            mount
        );
    }
}
...
fn test_non_system_mount_points() {
    ...
    for mount in safe_mounts {
        assert!(
            !is_system_mount_point(mount),
            "{} should NOT be detected as system mount",
            mount
        );
    }
}
    ...
    if let Ok(drives) = list_all_drives() {
        for drive in drives {
            if drive.is_system {
                ...
                assert!(result.is_err(), "Should reject system drive: {}", drive.path);
            }
        }
    }
}
