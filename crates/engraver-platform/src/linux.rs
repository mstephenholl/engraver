...
use crate::{align_up, is_aligned, DeviceInfo, OpenOptions, PlatformError, PlatformOps, RawDevice, Result};
...
impl RawDevice for LinuxDevice {
    ...
    fn write_at(&mut self, offset: u64, data: &[u8]) -> Result<usize> {
        ...
        if self.info.direct_io {
            ...
            if is_aligned(data.as_ptr() as usize, block_size)
            ...
            if let Some(ref mut buffer) = self.aligned_buffer {
                ...
                self.file.write(&aligned_slice[..aligned_len]).map_err(PlatformError::Io)
            } else {
